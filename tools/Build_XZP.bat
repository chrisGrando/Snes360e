...
:: Fixed by chrisGrando

@if exist Snes360.xzp del Snes360.xzp

@pushd "%~dp0..\snes9x-1.51-src-d3d\win32"
...
%XUIPKG% /d /a %XUI_XZP% "..\xbox\Skin\*" "..\xbox\Skin\Graphics\*"
...
@echo Adding non-graphics files from ..\xbox\Skin\ ...
...
@for %%f in ("..\xbox\Skin\*.xur") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
@for %%f in ("..\xbox\Skin\*.xui") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
@for %%f in ("..\xbox\Skin\*.xml") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
@for %%f in ("..\xbox\Skin\*.ttf") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
@for %%f in ("..\xbox\Skin\*.xma") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
@for %%f in ("..\xbox\Skin\*.3xui") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1

@REM Also copy non-graphics files from Graphics subdirectory (WAV files, etc.)
@if exist "..\xbox\Skin\Graphics" (
    @for %%f in ("..\xbox\Skin\Graphics\*.wav") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
    @for %%f in ("..\xbox\Skin\Graphics\*.xur") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
    @for %%f in ("..\xbox\Skin\Graphics\*.xui") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
    @for %%f in ("..\xbox\Skin\Graphics\*.xml") do @copy /Y "%%f" "%TEMP_SKIN%\" >nul 2>&1
)

@REM Add the filtered files to the XZP
%XUIPKG% /d /a %XUI_XZP% "%TEMP_SKIN%\*"
